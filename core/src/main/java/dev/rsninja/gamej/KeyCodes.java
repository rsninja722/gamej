...
public enum KeyCodes {
	...
	private static final Map<Integer, KeyCodes> CODES =
		new HashMap<>();

	private static final Map<String, KeyCodes> NAMES =
		new HashMap<>();

	static {
		for (final KeyCodes keyCode : values()) {
	...
	private KeyCodes(final int code) {
		this.code = code;
	}
	 ...
	 */
	public static KeyCodes get(final int code) {
		final KeyCodes keyCode = CODES.get(code);
	 ...
	 * code.
	 */
	public static KeyCodes get(final String name) {
		final KeyCodes keyCode = NAMES.get(name);
		if (keyCode == null) return UNDEFINED;
		return keyCode;
	}
